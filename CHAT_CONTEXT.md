...
## 4. Schedule & behavior
- Collections run on a **rolling 24h clock**: next sweep = the day's FIRST sweep + 24h
  (extra same-day runs never move the clock). Late boot = one catch-up now if the
  24h passed, else wait out the remainder. Never twice.
